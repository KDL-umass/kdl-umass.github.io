...
public class PowerPCCV extends SkeletonFinder {

    List<Double> esThresholds;
    PowerBayesNet trueBN;

    public PowerPCCV(Instances instances, PowerBayesNet trueBN, List<Double> thresholds) {
        super(instances);
        esThresholds = thresholds;
        this.trueBN = trueBN;
    }

    public PowerPCCV(Instances instances, PowerBayesNet trueBN, boolean useFixedInteval) {
        super(instances);
        this.trueBN = trueBN;
    ...
    public PowerPCCV(Instances instances, PowerBayesNet trueBN) {
    ...
    public int computeNeighbors() {
        ...
        //Now, run the PC algorithm on each trainFold with each threshold to determine which one performs best.
        for (int i = 0; i < trainFolds.size(); i++) {
            ...
            for (int j = 0; j < esThresholds.size(); j++) {
                ...
                FAS pc = new FAS(trainInstances, threshold);
                int sk_statCalls = pc.computeNeighbors();
                log.info("\t\tNumber of stat. calls=" + sk_statCalls);
        ...
        FAS pc = new FAS(instances, threshold);

        int finalStatCalls = pc.computeNeighbors();

        this.neighbors = pc.neighbors;
