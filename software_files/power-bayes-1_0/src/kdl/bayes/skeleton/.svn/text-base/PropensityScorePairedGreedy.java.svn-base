...
public class PropensityScorePairedGreedy extends SkeletonFinder {

    protected static Logger log = Logger.getLogger(PropensityScorePairedGreedy.class);
    ...
    public PropensityScorePairedGreedy(Instances trainData) {
        this(trainData, new ThresholdModule());
    }


    public PropensityScorePairedGreedy(Instances instances, ThresholdModule module) {
        super(instances);
        threshModule = module;
...
//            rf.setNumTrees(1);
            ...
            try {
                ...
                for (int i = 0; i < numTreatment; i++) {
                    ...
                    double prob = treatmentInstance.value(treatment.numAttributes() - 1);

                    double bestDiff = 5.0;
                    int bestInstance = -1;

                    for (int j = 0; j < nonTreatment.numInstances(); j++) {
                        Instance poolInstance = nonTreatment.instance(j);

                        double matchProb = poolInstance.value(nonTreatment.numAttributes() - 1);
