/**
 * $Id: StatUtil.java 281 2009-08-15 18:23:09Z afast $
 *
...
public final class StatUtil {
    ...
    /**
     * Computes the one-sided probability of seeing a value more extreme than k in n trials with expected proportion p
     *
     * @param k
     * @param n
     * @param p
...
//                                    (double) (n_ik[i][k] * n_jk[j][k])));
    ...
    public static double gStatistic(int xIdx, int yIdx, int[] zIdxs, BayesData data) {
        //

        ADTree adTree = data.getADTree();

        int x = 0;
        int aSize = data.computeCardinality(xIdx);
        int bSize = data.computeCardinality(yIdx);
        int cSize = data.computeCardinality(zIdxs);
        ...
        for (int i = 0; i < aSize; i++) {
            for (int j = 0; j < bSize; j++) {
                ZQueryIterator iter = new ZQueryIterator(zIdxs, data.instances);
                int k = 0;
                while (iter.hasNext()) {
                    ...
                    double count = adTree.getCount(query, 1);

                    //log.info("[" + i + "][" + j + "][," + query.get(2) + "] = " + count);
...
//                                    (double) (n_ik[i][k] * n_jk[j][k])));
    ...
    public static double gStatistic(SparseCounts counts) {
        //
        int x = 0;
        int aSize = counts.rows();
        int bSize = counts.columns();
        int cSize = counts.slices();
        ...
        for (int i = 0; i < counts.rows(); i++) {
            for (int j = 0; j < counts.columns(); j++) {
                for (int k = 0; k < counts.slices(); k++) {
                    n_ik[i][k] += counts.get(i, j, k);
                    n_jk[j][k] += counts.get(i, j, k);
                    n_k[k] += counts.get(i, j, k);
                }
            }
        }
...
//                                    (double) (n_ik[i][k] * n_jk[j][k])));
                            (
                                    (safeLog(counts.get(i, j, k)) + safeLog(n_k[k])) -
                                            (safeLog(n_ik[i][k]) + safeLog(n_jk[j][k]))
                            );
                }
            }
...
//        log.info("n=" + n);
//        log.info("n_k=" + Arrays.toString(n_k));
//        log.info("n_ik=" + Arrays.deepToString(n_ik));
//        log.info("n_jk=" + Arrays.deepToString(n_jk));
        ...
        for (int i = 0; i < counts.length; i++) {
            for (int j = 0; j < counts[i].length; j++) {
                for (int k = 0; k < counts[i][j].length; k++) {
                    mi += (counts[i][j][k] / n)
                            * ((safeLog(counts[i][j][k] / n) + safeLog(n_k[k] / n)) -
                            (safeLog(n_ik[i][k] / n) + safeLog(n_jk[j][k] / n)));
                }
            }
        }
        return mi;
    ...
    public static double mutualInformation(SparseCounts counts) {
        ...
        for (int i = 0; i < counts.rows(); i++) {
            for (int j = 0; j < counts.columns(); j++) {
                for (int k = 0; k < counts.slices(); k++) {
                    int cellCount = counts.get(i, j, k);
                    n_ik[i][k] += cellCount;
                    n_jk[j][k] += cellCount;
                    n_k[k] += cellCount;
                    n += cellCount;
...
//        log.info("n_jk=" + Arrays.deepToString(n_jk));
        ...
        for (int i = 0; i < counts.rows(); i++) {
            for (int j = 0; j < counts.columns(); j++) {
                for (int k = 0; k < counts.slices(); k++) {
                    double cellCount = counts.get(i, j, k);
                    mi += (cellCount / n)
                            * ((safeLog(cellCount / n) + safeLog(n_k[k] / n)) -
                            (safeLog(n_ik[i][k] / n) + safeLog(n_jk[j][k] / n)));
                }
            }
        }
    ...
    public static double mutualInformation(Map<Integer, Map<Integer, Map<Integer, Integer>>> counts) {

        int x = 0;
        //int aSize = counts.slices();
        //int bSize = counts.rows();
        //int cSize = counts.columns();

        Map<List<Integer>, Integer> n_ik = new HashMap<List<Integer>, Integer>();
        Map<List<Integer>, Integer> n_jk = new HashMap<List<Integer>, Integer>();
        Map<Integer, Integer> n_k = new HashMap<Integer, Integer>();
        //int[][] n_ik = new int[aSize][cSize];
        //int[][] n_jk = new int[bSize][cSize];
        //int[] n_k = new int[cSize];
        double n = 0.0;
        ...
        for (Integer i : counts.keySet()) {
            Map<Integer, Map<Integer, Integer>> jMap = counts.get(i);
            for (Integer j : jMap.keySet()) {
                Map<Integer, Integer> kMap = jMap.get(j);
                for (Integer k : kMap.keySet()) {
                    int cellCount = kMap.get(k);
                    List<Integer> ikList = new ArrayList<Integer>();
                    ikList.add(i);
                    ikList.add(k);
                    int n_ikCount = 0;
                    if (n_ik.containsKey(ikList)) {
                        n_ikCount = n_ik.get(ikList);
                    }
                    n_ikCount += cellCount;
                    n_ik.put(ikList, n_ikCount);


                    List<Integer> jkList = new ArrayList<Integer>();
                    jkList.add(j);
                    jkList.add(k);
                    int n_jkCount = 0;
                    if (n_jk.containsKey(jkList)) {
                        n_jkCount = n_jk.get(jkList);
                    }
                    n_jkCount += cellCount;
                    n_jk.put(jkList, n_jkCount);


                    int n_kCount = 0;
                    if (n_k.containsKey(k)) {
                        n_kCount = n_k.get(k);
                    }
                    n_kCount += cellCount;
                    n_k.put(k, n_kCount);
...
//        log.info("n_jk=" + Arrays.deepToString(n_jk));
        ...
        for (Integer i : counts.keySet()) {
            ...
            for (Integer j : jMap.keySet()) {
                Map<Integer, Integer> kMap = jMap.get(j);
                for (Integer k : kMap.keySet()) {
                    double cellCount = kMap.get(k);
                    List<Integer> ikKey = new ArrayList<Integer>();
                    ikKey.add(i);
                    ikKey.add(k);


                    List<Integer> jkKey = new ArrayList<Integer>();
                    jkKey.add(j);
                    jkKey.add(k);


                    mi += (cellCount / n)
                            * ((safeLog(cellCount / n) + safeLog(n_k.get(k) / n)) -
                            (safeLog(n_ik.get(ikKey) / n) + safeLog(n_jk.get(jkKey) / n)));
    ...
    /**
     * The Stuart-Maxwell Test is a general test for comparing the marginals of a square-contingency table.
     * For 2 x 2 tables, the Stuart-Maxwell is equivalent to McNemar's test.
     * <p/>
     * It is generally useful for comparing changes in paired or otherwise matched instances.
     *
     * @param counts
     * @return
     */
    public static double stuartMaxwell(int[][] counts) {

        //printCounts(counts);

        List<Integer> zeroMarginals = findZeroMarginalRowsColumns(counts);
        counts = shrinkCounts(zeroMarginals, counts);

        int numOutcomes = counts.length;

        //if(zeroMarginals.size() > 0){
        //    printCounts(counts);
        //}

        double pVal = 1.0;
        if (numOutcomes == 2) { //Binary outcomes, Run McNemars test, or exact binomial test
            double mcnemar = -1.0;

            int b = counts[0][1];
            int c = counts[1][0];

            if (b + c >= 30) {   //Use chi-square  approximation
                mcnemar = Math.pow(b - c, 2) / (b + c + 1);
                //log.info("BINARY: chisq=" + mcnemar);
                pVal = StatUtil.chiSquareP(mcnemar, 1);

            } else {  //For sufficiently small numbers use the two-sided exact test instead.
                int min = Math.min(b, c);
                pVal = 2 * StatUtil.binomialCDF(min, (b + c), 0.5);
                //log.info("EXACT: ");
                //if(pVal > 1.0){
                //printCounts(counts);
                //}
        ...
        } else if (numOutcomes == 3) {  //Special case: Fleiss and Everitt 1971.

            double chiSq = stuartMaxwell3way(counts);
            //log.info("THREE-WAY: chisq=" + chiSq);
            pVal = chiSquareP(chiSq, numOutcomes - 1);

        } else if (numOutcomes >= 4) {  //General case: num outcomes > 3

            //printCounts(counts);
            double chiSq = stuartMaxwellMultiway(counts);
            //log.info("GENERAL: chisq=" + chiSq);
            pVal = chiSquareP(chiSq, numOutcomes - 1);
    ...
    protected static void printCounts(int[][] counts) {
        int numOutcomes = counts.length;
    ...
    public static double stuartMaxwell3way(int[][] counts) {
        int numOutcomes = counts.length;
        Assert.condition(numOutcomes == 3, "number of outcomes must be three");

        //First compute marginals
        int[] n_i = new int[numOutcomes];
        int[] n_j = new int[numOutcomes];
        int n = 0;
        ...
        //Then compute d-vector.
        int[] d = new int[numOutcomes];
        for (int i = 0; i < numOutcomes; i++) {
            d[i] = n_i[i] - n_j[i];
        }

        double n23 = ((counts[1][2] + counts[2][1]) / 2.0);
        double n13 = ((counts[0][2] + counts[2][0]) / 2.0);
        double n12 = ((counts[0][1] + counts[1][0]) / 2.0);

        double num = (n23 * Math.pow(d[0], 2)) + (n13 * Math.pow(d[1], 2)) + (n12 * Math.pow(d[2], 2));
        double den = 2.0 * (n12 * n23 + n12 * n13 + n13 * n23);
    ...
    public static double stuartMaxwellMultiway(int[][] counts) {
        List<Integer> indicesToRemove = checkPerfectAgreement(counts);

        if (indicesToRemove.size() > 0) {
            counts = StatUtil.shrinkCounts(indicesToRemove, counts);
        }
        ...
        DenseDoubleMatrix2D dHat = new DenseDoubleMatrix2D(1, numOutcomes - 1);
        for (int i = 0; i < numOutcomes - 1; i++) {
            dHat.set(0, i, d[i]);
        }

        DenseDoubleMatrix2D vHat = new DenseDoubleMatrix2D(numOutcomes - 1, numOutcomes - 1);

        for (int i = 0; i < numOutcomes - 1; i++) {
            for (int j = 0; j < numOutcomes - 1; j++) {
                if (i == j) {
                    int v = n_i[i] + n_j[j] - 2 * counts[i][j];
                    vHat.set(i, j, v);
                } else {
                    int v = -1 * (counts[i][j] + counts[j][i]);
                    vHat.set(i, j, v);
                }
            }
        }
        ...
        try {
            DoubleMatrix2D step1 = alg.mult(dHat, alg.inverse(vHat));

            chiSq = alg.mult(step1, alg.transpose(dHat)).get(0, 0);
    ...
    protected static List<Integer> checkPerfectAgreement(int[][] counts) {
        // Need to check counts for perfect agreement and remove that column and row.
        List<Integer> indicesToRemove = new ArrayList<Integer>();
        for (int i = 0; i < counts.length; i++) {
            boolean isPerfect = true;
            for (int j = 0; j < counts.length; j++) {
                if (i != j && (counts[i][j] > 0 || counts[j][i] > 0)) {
                    isPerfect = false;
                    break;
                }
            }
    ...
    /**
     * Returns a list of indices indicating which row/column combinations have no values.
     *
     * @param counts
     * @return
     */
    protected static List<Integer> findZeroMarginalRowsColumns(int[][] counts) {
        // Need to check counts for zeros in an entire row and column and remove that column and row.
        List<Integer> indicesToRemove = new ArrayList<Integer>();
        for (int i = 0; i < counts.length; i++) {
            boolean isZero = true;
            for (int j = 0; j < counts.length; j++) {
    ...
    protected static int[][] shrinkCounts(List<Integer> indicesToRemove, int[][] counts) {
        int newSize = counts.length - indicesToRemove.size();
        int[][] newCounts = new int[newSize][newSize];
        ...
        for (int i = 0; i < counts.length; i++) {
            ...
            for (int j = 0; j < counts.length; j++) {
                ...
                newCounts[newI][newJ] = counts[i][j];
                newJ++;
            }
            newI++;
        }
